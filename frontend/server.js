...
// middleware to ensure the user is logged in and is authority
...
app.post('/login', async (req, res) => {
    ...
    if (user) {
        //this will regenerate the session id after successful login
        req.session.regenerate((err) => {
          ...
          // this will store the user details in he session
          req.session.user = { username, role };
          res.redirect(role === 'student' ? '/student-profile' : '/parent-profile');
        });
      } else {
...
app.get('/logout', (req, res) => {
    req.session.destroy((err) => {
      ...
      res.redirect('/login'); // this will redirect to the login page after logout
    });
  });
...
app.get('/notice-board', isAuthenticated, authenticateStudent, async (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'notice_board.html'));
});

//personal info viewing endpoint (student profile)
app.get('/personal-info', isAuthenticated, authenticateStudent, async (req, res) => {
...
app.get('/family-info', isAuthenticated, authenticateStudent, async (req, res) => {
...
//attendance endpoint (authorities' profile)
...
//noticeboard endpoint (authorities' profile)
...
//room cleaning endpoint (authorities' profile)
